...
def check_if_won():
    ...
    # Check for O
    elif b1["text"] == "O" and b2["text"] == "O" and b3["text"] == "0":
        b1.config(bg="red")
        b2.config(bg="red")
        b3.config(bg="red")
        winner = True
    ...
    if count == 9 and winner is False:
        messagebox.showinfo("Tic Tac Toe", "It a Tie!\n No one wins")
        disable_all_buttons()
...
def b_click(b):
    ...
    if b["text"] == " " and clicked is True:
        ...
        messagebox.showerror("Tic Tac Toe", "Hey that box has been already been selected\nPick another box...")
...
my_menu = Menu(root)
# Tkinter know how a menu looks and does all the work in background we don't have to define it
root.config(menu=my_menu)

# Let's create options menu
options_menu = Menu(my_menu, tearoff=False)  # tearoff for no dotted lines
my_menu.add_cascade(label="Options", menu=options_menu)  # Creating submenus
options_menu.add_command(label="Reset Game", command=reset)  # Adding things to submenu
# Separating diff options by a bar
options_menu.add_separator()
options_menu.add_command(label="Quit Game", command=root.quit)
reset()
